...
(function($, window, document, undefined) {
	...
	jRange.prototype = {
		defaults: {
			...
			width: 300,
			disable: false,
			isMstep:false,     //自己添加   用于设置class="pointer high"是否可以拉动 ，同时左右点击是移动距离
		},
		...
		barClicked: function(e) {
			...
			var x = e.pageX - this.clickableBar.offset().left,
				y=(e.pageX - this.clickableBar.offset().left)/this.defaults.width,
                s = this.options.scale || [this.options.from, this.options.to];

            if (this.isSingle()){
				if(this.options.isMstep){     //isMstep 来设置移动距离
					var _index=parseInt(x*(s.length-1)/this.options.width),
                        num;
					if(x<parseInt($(".pointer.high").css("left"))){
                        num=_index*(this.options.width/(s.length-1));
                    }else{
                        num=(_index+1)*(this.options.width/(s.length-1));
                    }
                    this.setPosition(this.pointers.last(), num, true, true);
                }else{
